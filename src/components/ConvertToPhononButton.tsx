...
export function ConvertToPhononButton() {
  ...
  const updateBalances = useCallback(async () => {
    if (
      typeof ethersContext.ethersProvider === "undefined" ||
    ...
    let gridAllowance: ethers.BigNumber = await gridToken.allowance(
      address,
      REDEEMER_CONTRACT_ADDRESS
    );
  ...
  const expectedPhonon = useMemo(() => {
    ...
    if (gridDecimals < phononDecimals) {
      expectedBalance =
        expectedBalance * BigInt(Math.pow(10, phononDecimals - gridDecimals));
    }

    if (phononDecimals < gridDecimals) {
      expectedBalance =
        expectedBalance / BigInt(Math.pow(10, gridDecimals - phononDecimals));
    }
  ...
  return (
    <div className="flex flex-col text-center md:mx-24">
      ...
      {ethersContext.active === false && (
        <>
          <button
            className="bg-gradient-to-br from-green-200 via-indigo-200 to-pink-200 text-black rounded p-2 px-5 shadow-lg shadow-indigo-300/40 hover:shadow-indigo-300/70  hover:-hue-rotate-90 hover:shadow-lg ease-in-out duration-300"
            onClick={() => {
      ...
      {ethersContext.active === true && (
        <>
          ...
          {gridBalance > BigInt(0) && (
            <>
              <h4 className="text-sm text-pink-200 mt-5 uppercase mb-5">
                Connected as {address?.substring(0, 8)}...
                {address?.substring(34)}
              </h4>
              <p className="text-2xl mb-7">
                You have{" "}
                <span className="text-indigo-200">
                  {displayBalance(gridBalance, gridDecimals)} GRID
                </span>
              </p>
              {gridAllowance < gridBalance && (
                <button
                  ...
                  onClick={async () => {
                    try {
                      setIsLoading(true);
                      let tx: TransactionResponse = await gridToken.approve(
                        REDEEMER_CONTRACT_ADDRESS,
                        gridBalance
                      );
                      ...
                      setIsLoading(false);
                      setError(
                        "Error approving. Please try again or contact support."
                      );
                    }
                  }}
                >
                  {isLoading === true
                    ? "Approving..."
                    : "Allow PHONON to convert GRID tokens"}
                </button>
              )}
              {gridAllowance >= gridBalance && (
                <button
                  ...
                  onClick={async () => {
                    try {
                      ...
                      setIsLoading(false);
                      setError(
                        "Error redeeming tokens. Please try again or contact support."
                      );
                    }
                  }}
                >
          ...
          {gridBalance === BigInt(0) && phononBalance > BigInt(0) && (
            <p className="my-3">
              ...
              {displayBalance(phononBalance, phononDecimals)} PHONON
            </p>
          )}
